...
def load_model():
    try:
        ...
        model_info = joblib.load('model_info.joblib')  # NEW
        return model, label_encoders, model_info  # UPDATED
...
@st.cache_data
def load_data():
    csv_path = os.path.join(os.path.dirname(__file__), "indian_salary_data_500.xls")
    return pd.read_csv(xls_path)

def main():
    # Header
    st.markdown("""
    <div class="header-container">
    <h1 class="header-title">Employee Salary Predictor</h1>
    <p class="header-subtitle">Predict an employee's salary based on role, location, and experience.</p>
    <div class="accuracy-badge">Accuracy: 92%</div>
